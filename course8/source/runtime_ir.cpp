...
// 将当前op计算的结果（output）放到nextop的input上
...
RuntimeGraph::Forward(const std::vector<std::shared_ptr<Tensor<float>>> &inputs, bool debug) {
    // 检查当前的执行图是否已经初始化完毕
