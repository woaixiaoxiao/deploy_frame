...
void Tensor<float>::Padding(const std::vector<uint32_t> &pads, float padding_value) {
    ...
    // 请补充代码
    // 先将原来的数据保留
    arma::fcube old_data = this->data_;
    // 构造一个新的形状的数据
    this->data_ = arma::fcube(this->rows() + pad_rows1 + pad_rows2,
    ...
    for (uint32_t i = 0; i < channels; i++) {
        // 先将数据全部设置为padding_value
        auto &cur_data = this->data_.slice(i);
        cur_data.fill(padding_value);
        // 然后将原始的数据恢复
        for (int j = pad_rows1; j < rows - pad_rows2; j++) {
            for (int k = pad_cols1; k < cols - pad_cols2; k++) {
                cur_data.at(j, k) = old_data.slice(i).at(j - pad_rows1, k - pad_cols1);
            }
        }
    }
    //    LOG(INFO) << "padding function: " << padding_value;
}
