...
void RuntimeGraph::Build(const std::string &input_name, const std::string &output_name) {
    ...
    this->ReverseTopo2(operators_maps_.at("pnnx_output_0"));
    //    for (const auto &[_, op] : operators_maps_) {
    //        // 根据输入节点构建拓扑排序
    //        if (op->type == "pnnx.Input" && !op->has_forward) {
    //            LOG(INFO) << "hello here!!!!!!!!!!!!!!!!!!!!!" << op->name;
    //            this->ReverseTopo(op);
    //        }
    //    }
...
// 增加了反向拓扑排序
void RuntimeGraph::ReverseTopo2(const std::shared_ptr<RuntimeOperator> &root_op) {
    CHECK(root_op != nullptr) << "current operator is nullptr";
    // 首先取出输入的所有操作数，根据输入操作数的name找到生成这个操作数的op
    const auto &input_operands = root_op->input_operands_seq;
    for (const auto &input_operand : input_operands) {
        const auto &name = input_operand->name;
        const auto &pre_op = this->operators_maps_.at(name);
        if (pre_op != nullptr) {
            if (!pre_op->has_forward) {
                this->ReverseTopo2(pre_op);
            }
        }
    }
    root_op->has_forward = true;
